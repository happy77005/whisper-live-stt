...
def process_audio(file_path):
    try:
        ...
        # Remove vocals from the original audio to isolate non-vocal parts (instruments)
        non_vocals = samples - vocals

        # Ensure no distortion by placing vocals strictly on the right and non-vocals strictly on the left
        left_channel = np.copy(non_vocals)  # Instruments on the left
        right_channel = np.copy(vocals)  # Vocals on the right

        # Combine the left and right channels into a stereo output
